...
const site = ref('')
const checkSite = ref('')
const { data, pending, error, execute } = await useFetch(() => `/api/about?site=${checkSite.value}`, { immediate: false, retry: false })

function check() {
  if(checkSite.value === site.value) {
    execute()
  } else {
    checkSite.value = site.value
  }
}
</script>
...
<template>
  ...
  <p class="error" v-if="error">Something went wrong, maybe you hit the 10 second Vercel cap or maybe I'm out of money. Please try again?</p>
</template>
